...
export function mapBillingError(error: ErrorLike) {
  ...
  if (code === 'PGRST116' || code === 'P0002') {
    return new AppError('not-found', 'Mensalidade nao encontrada.', error.message)
  }

  if (code === '23505') {
    return new AppError('validation', 'Ja existe uma mensalidade ativa para este aluno no mes.', error.message)
  }
  ...
  if (
    ...
    code === '22023' ||
    code === '22P02' ||
    message.includes('exceeds monthly fee balance')
  ) {
    return new AppError('validation', 'Revise os valores financeiros e tente novamente.', error.message)
  }
